...
def gen_datapoint(fname_disparity, fname_disparity_next_frame, fname_disparity_change, fname_optical_flow, image, image_next_frame, n = 8192, max_cut = 35, focal_length=1050.):
    ...
    depth_np = cv2.imread(depth, cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH)[:, :, ::-1] / 255.
    flow_np = cv2.imread(flow)[:, :, ::-1] / 255.
    rgb_next_np = cv2.imread(rgb_next)[:, :, ::-1] / 255.
    depth_next_np = cv2.imread(depth_next, cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH)[:, :, ::-1] / 255.
    flow_next_np = cv2.imread(flow_next)[:, :, ::-1] / 255.  
    # new read process  
