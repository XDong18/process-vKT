...
def gen_datapoint(fname_disparity, fname_disparity_next_frame, fname_disparity_change, fname_optical_flow, image, image_next_frame, n = 8192, max_cut = 35, focal_length=1050.):
    ...
    flow_next_np = (cv2.imread(flow_next_fn, cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH)[:, :, ::-1]  * 2.0 / 65535.0 - 1.0) * 10.0
    # new read process  
    ...
    future_pix1_x = sampled_pix1_x + flow_np[:,:,0]
    future_pix1_y = sampled_pix1_y + flow_np[:,:,1]
    ...
    current_pos2 = np.array([get_3d_pos_xy( sampled_pix2_y[i], sampled_pix2_x[i], depth_next_frame_np[int(sampled_pix2_y[i]), int(sampled_pix2_x[i])], w, h) for i in range(n)])
    current_rgb2 = np.array([[rgb_next_np[int(sampled_pix2_y[i]), int(sampled_pix2_x[i]), 0], rgb_next_frame_np[int(sampled_pix2_y[i]), int(sampled_pix2_x[i]), 1], rgb_next_frame_np[int(sampled_pix2_y[i]), int(sampled_pix2_x[i]), 2]] for i in range(n)])
    ##### point set 2 current pos
