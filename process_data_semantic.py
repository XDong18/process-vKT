...
def gen_datapoint(semantic_fn, rgb_fn, depth_fn, rgb_next_fn, depth_next_fn, flow_fn, n = 8192, max_cut = 100, focal_length=725.0087):
    np.random.seed(0)
    ##### generate needed data

    # new read process
    semantic_np = cv2.imread(semantic_fn)[:, :, ::-1]
    ...
    current_class_rgb1 = np.array([[semantic_np[int(sampled_pix1_y[i]), int(sampled_pix1_x[i]), 0], semantic_np[int(sampled_pix1_y[i]), int(sampled_pix1_x[i]), 1], semantic_np[int(sampled_pix1_y[i]), int(sampled_pix1_x[i]), 2]] for i in range(n)])
    current_class1 = np.zeros((current_class_rgb1.shape[0], 1))
    for idx, class_rgb in enumerate(current_class_rgb1):
        tuple_rgb = (class_rgb[0], class_rgb[1], class_rgb[2])
        if CLASSES[tuple_rgb] in ['Truck', 'Car', 'Van']:
            current_class1[idx] = 1
    ...
    return current_pos1, current_pos2, current_rgb1, current_rgb2, flow, mask1, current_class1
...
def proc_one_scene_vkt(scene_root, out_dir):
    ...
    semantic_root = os.path.join(frame_root, 'classSegmentation', 'Camera_0')
    for rgb_fn in sorted(os.listdir(rgb_root)):
        ...
        if len(glob.glob(os.path.join(semantic_root, '*' + str_idx + '*'))) == 0:
            continue
        
        semantic_fn = glob.glob(os.path.join(semantic_root, '*' + str_idx + '*'))[0]
        ...
        d = gen_datapoint(semantic_fn, rgb_fn, depth_fn, rgb_next_fn, depth_next_fn, sceneflow_fn)
        if d is not None:
...
def main():
    ...
    parser.add_argument('--output_dir', default='data_processed_vkt2_semantic', type=str, help='output dir')
